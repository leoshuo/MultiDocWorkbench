# Document Workspace 1.6.6 项目总结与评审文档
...
#### 填入摘要替换逻辑
- **替换而非追加**：填入摘要操作现在会替换目标位置的内容，而不是追加到现有内容后面
- **沉淀与 Replay 一致**：录制沉淀和 Replay 执行时都使用替换逻辑
- **多次 Replay 不重复**：避免内容累积问题

#### AI 执行指导增强
- **计算类任务支持**：Replay 时自动检测并执行计算类指令（如求和、公式计算）
- **多来源解析**：`aiGuidance` 字段支持从 llmScript、meta、llmRecordContent、content 多个来源获取
- **智能数值提取**：AI 能从文本中提取数值并按公式计算（如 "68+89+373+29=559"）
...
- **aiGuidance 解析**：修复【AI执行指导】字段在保存和 Replay 时解析失败的问题
