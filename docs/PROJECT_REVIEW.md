...
### 新增功能
- **沉淀双记录机制**: 每个 Section 同时保存 llmScript（大模型记录）和 originalScript（脚本记录）
- **智能 Replay 优先级**: 大模型模式优先使用 llmScript，失败后自动回退到脚本执行
- **Replay 状态细化**: 新增 `llm_done`、`script_done`、`skipped` 状态标识
- **对话持久化**: 切换工作台时对话记录保持不变（服务端缓存）
- **清除对话按钮**: 对话面板新增清除按钮
- **AI 对话增强**: 问答功能基于已加载文档内容智能回复

### UI/UX 优化
- **应用端专业主题**: 全新渐变色彩主题，更加现代专业
- **沉淀列表双记录展示**: 展开 Section 时同时显示大模型记录和脚本记录
- **确认保存按钮样式**: 翠绿渐变，始终可点击

### 技术改进
- 场景 404 自动创建机制
- API Key 多种配置方式（环境变量/命令行/运行时 API）
- 沉淀编译容错处理
