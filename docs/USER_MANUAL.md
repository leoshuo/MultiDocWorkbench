...
- **沉淀双记录机制**：每个 Section 同时保存大模型记录（llmScript）和脚本记录（originalScript）
- **智能 Replay 优先级**：优先使用大模型 Replay，失败后自动回退到脚本 Replay
- **Replay 状态细化**：区分 `llm_done`、`script_done`、`skipped`、`fail` 状态
- **对话持久化**：切换工作台时对话记录保持不变
- **UI 专业化**：应用端工作台全新渐变主题，更加现代专业
